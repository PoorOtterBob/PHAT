...
import time
from ts_benchmark.baselines.phat.models.phat_model import PHATModel
from ts_benchmark.baselines.phat.utils.losses import fredf_loss
from ts_benchmark.baselines.phat.utils.tools import EarlyStopping, adjust_learning_rate
...
class PHAT(ModelBase):
    def __init__(self, **kwargs):
        super(PHAT, self).__init__()
        self.config = PHATConfig(**kwargs)
    ...
    def forecast_fit(
        ...
        **kwargs,
        ...
        self.model = PHATModel(self.config)
        ...
        if config.loss == "MSE":
            ...
            criterion = nn.L1Loss()
        elif config.loss == "Fre":
            criterion = fredf_loss()
        else:
        ...
        for epoch_num, epoch in enumerate(range(config.num_epochs)):
            self.model.train()
            # for input, target, input_mark, target_mark in train_data_loader:
            for i, (input, target, input_mark, target_mark) in tqdm(enumerate(
                train_data_loader
                ...
                output = self.model(input, epoch_num<config.pre_epochs)
            ...
            if epoch_num >= config.pre_epochs:
